...
CDRMTransmitter::CDRMTransmitter(CSettings *nPsettings)
    : CDRMTransceiver(nPsettings), ReadData(), TransmitData(),
...
bool CDRMTransmitter::CanSoftStopExit() {
  ...
  if (bNewSymbol) {
    ...
    const bool bStopRequested =
        false; // Parameters.eRunState != CParameter::RUNNING;
...
void CDRMTransmitter::Init() {
  ...
  Parameters.InitCellMapTable(Parameters.GetWaveMode(),
                              Parameters.GetSpectrumOccup());
...
void CDRMTransmitter::LoadSettings() {
  ...
  Parameters.SetNewAudSampleRate(
  ...
  for (int i = 0; i < 1 /*MAX_NUM_SERVICES*/; i++) // TODO
  {
    ...
    /* Service description */
    Service.iServiceDescr =
        s.Get(service, "description", int(Service.iServiceDescr));
